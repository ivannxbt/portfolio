...
async function optimizeImage(config) {
  ...
  try {
    ...
    console.log(
      `  Original: ${originalSizeKB} KB -> WebP: ${webpSizeKB} KB (${savings}% savings)`,
    );
...
async function main() {
  ...
  console.log(
    `Project Images: ${(projectResults.reduce((s, r) => s + r.originalSize, 0) / 1024 / 1024).toFixed(2)} MB -> ${(projectResults.reduce((s, r) => s + r.webpSize, 0) / 1024 / 1024).toFixed(2)} MB`,
  );
  console.log(
    `Social Icons: ${(socialResults.reduce((s, r) => s + r.originalSize, 0) / 1024 / 1024).toFixed(2)} MB -> ${(socialResults.reduce((s, r) => s + r.webpSize, 0) / 1024 / 1024).toFixed(2)} MB`,
  );
  console.log(
    `\nTotal: ${(totalOriginal / 1024 / 1024).toFixed(2)} MB -> ${(totalWebP / 1024 / 1024).toFixed(2)} MB`,
  );
  console.log(
    `Savings: ${((totalOriginal - totalWebP) / 1024 / 1024).toFixed(2)} MB (${totalSavings}%)`,
  );
